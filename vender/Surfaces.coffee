...
class Surfaces
  _ = window["_"]
  @regMap = (reg, str, iterate)->
    arr = []
    while true
      matches = reg.exec(str)
      if not matches? then break
      arr.push(iterate(matches))
    arr
  @range = (head)->
    regHead = ///
      (\!)?         #flag
    ...
    rjcts = []
    aplys = []
    Surfaces.regMap regHead, head, ([__, a, b, c])->
      flag = a is undefined
      start = Number(b)
      stop  = Number(c)
      nums = if isFinite b then [a..b] else [a]
      if flag then rjcts = rjcts.concat(nums)
  ...
  constructor: (text)->
    ...
    @_defAry = []
    @_apdAry = []
    Surfaces.regMap regComment, text, ([match])-> #commentout
      text = text.replace(match, "")
    Surfaces.regMap regOldIntvl, text, ([match,
                                 animId,
      ...
      text = text.replace match,
        ...
        ".interval," +
        "#{timing}"
    Surfaces.regMap regOldPtn, text,([match,
                              anmId,
                              ptnId,
                              srfId,
                              wait,
  ...
  class Collisions
    ...
    constructor: (body)->
      return @ unless body?
      Surfaces.regMap reg, body, ([__, n, sx, sy, ex, ey, id])=>
  ...
  class Elements
    ...
    constructor: (body)->
      return @ unless body?
      Surfaces.regMap reg, body, ([__, n, layer, src, x, y])=>
  ...
  class Patterns
    ...
    ///g
    constructor: (body)->
      Surfaces.regMap ptnReg, body, ([__, n, layer, num, minWait, maxWait, x, y])=>
  ...
  class Animations
    ...
    constructor: (body)->
      return @ unless body?
      anmN = {}
      Surfaces.regMap reg, body, ([__, n, str])->
        n = Number n
        anmN[n] = (anmN[n] or "") + "\r\n" + str
      for n, body of anmN
