...
module cuttlebone {
  ...
  function serverScript(): ()=>void{
    return (()=>{
      ...
      var handlers:{[key:string]: (data:any, reply:(arg:any[], trf:ArrayBuffer[])=>void)=>void} = {};
      self.addEventListener("message", (ev)=>{
        var {data:[id, event, data]} = <{data:[number, string, any]}>ev;
  ...
  export class BlobWorker {
    ...
    request(event:string, tuple:(()=>void)|[any, ArrayBuffer[]], callback?:()=>void): void{
